...
- **`runbook`** — `argus runbook`. Runbook management stored as YAML in `~/.argus/runbooks/`. Subcommands: `create`(`init` for samples)/`list`/`show`/`search`/`validate`/`run`/`delete`. Steps can be automated (command), check-based, or manual. `run` is a dry run by default (steps are shown, nothing executes); `--execute` actually runs command/check steps after a per-step confirmation, with timeouts, captured output, and a run log saved under `~/.argus/runbooks/runs/`. A failed run log exits 1.
